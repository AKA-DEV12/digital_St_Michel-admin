...
@section('content')
    <div class="mb-5 animate-fade-in">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h1 class="h3 fw-bold mb-1">Nouvelle Critique</h1>
                <p class="text-secondary">Ajoutez une nouvelle note et un avis pour la colonne centrale.</p>
            </div>
    ...
    <form action="{{ route('admin.reviews.store') }}" method="POST" enctype="multipart/form-data" class="animate-fade-in" style="animation-delay: 0.1s">
        @csrf
        <div class="row g-4">
            <div class="col-12 col-lg-8">
                <div class="card border-0 rounded-4 shadow-sm p-4 overflow-hidden bg-white">
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" class="form-control rounded-3 border-gray-100 py-3 px-4 @error('title') is-invalid @enderror" placeholder="Ex: At vero eos et ccusamus" value="{{ old('title') }}" required>
                        @error('title') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-4">
                        <label class="form-label fw-bold text-dark small text-uppercase">Note / Score (sur 10)</label>
                        <input type="number" step="0.1" name="score" class="form-control rounded-3 border-gray-100 py-3 px-4 @error('score') is-invalid @enderror" placeholder="Ex: 8.4" value="{{ old('score', 0) }}" required>
            ...
            <div class="col-12 col-lg-4">
                    ...
                    <div class="mb-4">
                        <div class="image-upload-wrapper text-center p-4 border-2 border-dashed border-gray-100 rounded-4 bg-light transition-all hover-bg-light-2">
                            ...
                            <label for="imageInput" class="cursor-pointer">
                                ...
                                <p class="small text-secondary fw-medium mb-0">Cliquez pour ajouter une image</p>
                        ...
                        <div id="imagePreview" class="mt-3 rounded-3 overflow-hidden d-none">
                            <img src="" class="w-100 h-auto" id="previewImg">
                        </div>
                    </div>
                </div>
                ...
                <div class="d-grid gap-2 mt-4">
                    <button type="submit" class="btn btn-primary rounded-3 py-3 fw-bold shadow-sm d-flex align-items-center justify-content-center gap-2">
                        <i class="fa-solid fa-save"></i> ENREGISTRER LA CRITIQUE
                    </button>
                    <button type="reset" class="btn btn-light rounded-3 py-3 fw-bold text-secondary border border-gray-100 h-12 flex items-center justify-center">
                        RÉINITIALISER
                    </button>
                </div>
            </div>
