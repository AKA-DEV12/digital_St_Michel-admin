   ...
   public function index()
{
    ...
    // Sécurité au cas où la migration MassRequest n'a pas encore tourné
    $hasMassRequests = \Schema::hasTable('mass_requests');
    ...
    $stats = [
        ...
        'total_mass_requests' => $hasMassRequests ? \App\Models\MassRequest::count() : 0,
        'pending_mass_requests' => $hasMassRequests ? \App\Models\MassRequest::where('status', 'pending')->count() : 0,
        ...
        'total_revenue' => \App\Models\Reservation::where('status', 'validated')->sum('price') + 
                           ...
                           ($hasMassRequests ? \App\Models\MassRequest::where('status', 'confirmed')->sum('amount') : 0),
        
        'total_children' => \App\Models\GroupMember::sum('nombre_enfant') ?? 0,
    ];
    ...
    // Sécurisation du calcul des âges
    ...
    foreach ($members as $m) {
        // Force la conversion en Carbon au cas où le cast est manquant dans le Model
        $dateNaissance = $m->date_naissance instanceof Carbon ? $m->date_naissance : Carbon::parse($m->date_naissance);
        $age = $dateNaissance->age;

        if ($age < 15) $enfants++;
        elseif ($age <= 35) $jeunes++;
        else $adultes++;
    }
    ...
    $recent_mass_requests = $hasMassRequests ? \App\Models\MassRequest::latest()->take(5)->get() : collect();
